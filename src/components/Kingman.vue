<template>
  <v-container>
    ...
    <v-container class="control">
      <v-row>
        <v-col>
          <v-slider label="\( c_s \)" hint="Service rate variation" min="0.1" max="5" step="0.1"
            v-model="c_s">
            <template v-slot:append>
              <span>{{ c_s.toFixed(1) }}</span>
            </template>
          </v-slider>
        </v-col>
        <v-col>
          <v-slider label="\( c_a \)" hint="Arrival rate variation" min="0.1" max="5" step="0.1"
            v-model="c_a">
            <template v-slot:append>
              <span>{{ c_a.toFixed(1) }}</span>
            </template>
          </v-slider>
        </v-col>
      </v-row>
      <v-row>
        <v-slider label="\( \mu \)" hint="Service rate" min="1" max="10" step="0.25" v-model="mu">
          <template v-slot:append>
    ...
    <v-row class="info">
      <v-col>
        ...
        <v-row>
          <p>\[ \mathbb {E} [W_q] \approx \left( \frac {\rho} {1 - \rho} \right) \left( \frac {c_a^2 + c_s^2} {2}
            \right) \tau \]</p>
        </v-row>
      </v-col>
      <v-col>
